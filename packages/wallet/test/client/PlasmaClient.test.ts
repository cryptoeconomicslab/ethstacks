...
describe('PlasmaClient', () => {
  ...
  it('deserialize ok', () => {
    ...
    const okString = PlasmaClient.deserialize<string>({result: ok.ok()}, result => result)
    assert.equal(okString.ok(), 'ok')
  })

  it('deserialize error', () => {
    const error = new ChamberError(100, 'error')
    const deserializedError = PlasmaClient.deserialize<string>(error.serialize(), result => result)
