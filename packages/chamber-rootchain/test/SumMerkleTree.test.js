...
contract('SumMerkleTreeTest', function ([user, owner, recipient, user4, user5]) {
  ...
  describe('checkMembership', function () {
    ...
    it('should be failed to checkMembership because of invalid left offset', async function() {
      const tree = new SumMerkleTree(leaves);
      const root = tree.root();
      const proof = tree.proof(leaves[2]);
      const result = await this.sumMerkleTree.checkMembership(
    ...
    it('should be failed to checkMembership because of non-inclusion', async function() {
      const tree = new SumMerkleTree(leaves);
      const root = tree.root();
      const proof = tree.proof(leaves[2]);
      const result = await this.sumMerkleTree.checkMembership(
        leaves[0].getLengthAsBigNumber().toNumber(),  // leaf amount
        utils.bufferToHex(leaves[0].getHash()),       // leaf hash
        2,
        24, // total deposit
        5, // left offset
        utils.bufferToHex(root),
